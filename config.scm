                  ...
                  (let* ([call (car calls)]
                    ...
                    (cond
                      ...
                      [(equal? name "context_wait")
                       ...
                       (define (unknown ids)
                         (cond [(null? ids) #f]
                               [(not (context-job-find context-jobs (car ids)))
                                (car ids)]
                       ...
                       (define pending
                         (if unknown-id '() (collect-pending requested)))
                       (if unknown-id
                           (begin
                             (set! messages
                                   (append messages
                                           (list
                                             (context-tool-result
                                               ...
                                               (hash 'error
                                                     (string-append
                                                       "unknown context compaction job: "
                                                       unknown-id))))))
                             (set! next-state
                                   (make-state messages compactions last-usage
                                               context-budget model reasoning
                                               service-tier "working" "" 0))
                             (request-effect messages last-usage model reasoning
                                             service-tier))
                           (if (null? pending)
                               (begin
                                 (set! messages
                               ...
                               (begin
                                 (set! next-state
                                       (make-state messages compactions last-usage
                                                   context-budget model reasoning
                                                   service-tier "waiting_context" "" 0))
                      ...
                      [else
                       ...
                       (define selected (context-selection active-context-items ids))
                       (if (context-selection-reserved? ids context-jobs)
                           (error! "context selection overlaps a pending compaction job")
                           (begin
                             (define job-id
                               (context-runtime-id "J" next-context-job))
                             (define job
                               (hash 'id job-id 'items ids
