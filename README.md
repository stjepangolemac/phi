...
`agent(prompt, { label?, schema?, branch?, branch_off?, model?, reasoning?, timeout_ms?, capabilities? })` allocates a durable child session in the same Phi home before starting a one-shot Phi child. Omitted model and reasoning values inherit the parent's effective selection. Explicit values are validated together against the parent session's pinned model catalog before the child process starts; changing the model uses that model's default service tier. A schema requests strict JSON-schema output and makes `agent()` return the parsed JSON value.

`timeout_ms` is an integer from 1 through 3,600,000. It covers queueing, managed-worktree setup, session creation, model requests, and tool execution, and is clamped below the workflow's remaining 60-minute deadline so cleanup can finish deterministically. A timed-out child records the distinct terminal status `timed_out`, is cancelled, and releases any managed worktree. The `capabilities` profile defaults to `parent`; `read-only` removes shell and write authority, while `workspace-write` keeps workspace file editing but removes shell and full-access authority. `workspace-write` requires parent write authority, and every profile is validated before launch so workflow code can preserve or attenuate, never expand, parent authority. RPC children are non-interactive, so an approval-required operation fails instead of waiting for input.

The initial runtime limits are 8 concurrent agents, 32 agents total per workflow, and 60 minutes per workflow. Workflow runs are distinct from definitions: task files, progress, logs, results, managed-worktree ownership metadata, and child relationships live under `$PHI_HOME/sessions/<parent-session-id>/workflows/<task-id>/`. Every launch receives a unique task ID and directory, including launches of same-named definitions. Each child has a separate flat `$PHI_HOME/sessions/<child-session-id>/` entry recording its parent session, workflow task, agent label, branch, workspace, actual worktree path, effective model/reasoning/service tier, timeout, and capability profile. Background tasks live for the duration of the parent Phi process and are cancelled when it exits, but their run records and child sessions remain durable.

For a single focused child, the plugin bundles `delegate`. It takes a required `prompt` plus optional `options` matching the currently supported `agent()` options (`label`, `schema`, `branch`, `branch_off`, `model`, `reasoning`, `timeout_ms`, and `capabilities`) and returns the child result directly:
