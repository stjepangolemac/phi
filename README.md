...
The official `context-management` plugin exposes three generic model-facing tools. `context_mark` closes the current raw span and starts a labeled span at a tool-safe boundary. `context_inspect` reports provider-anchored or estimated usage, fixed prompt tokens, and the ordered active `S*` raw and `C*` summary items. `context_compact` replaces one or more ordered, adjacent, closed items with one summary while retaining direct and nested provenance. The fixed prompt and current open span are never selectable. Providers may return multiple independent context-tool calls in one response; Phi preserves their call IDs and processes them in returned order across state barriers. Disjoint compactions run independently, while invalid or overlapping selections receive only their own error result. These boundaries are independent of planning and can represent investigation, implementation, debugging, review, user-directed scope changes, or any other focus shift. When selective compaction is available, model-facing pressure notices are advisory at 25%, encouraged at 50%, and high priority at 75%; a jump emits only the highest newly crossed notice, and compaction resets the next applicable threshold. Automatic threshold compaction and `/compact` remain the unchanged safety net when no markers exist, and immutable session events retain the complete raw history while only the active model-facing projection is reduced.
